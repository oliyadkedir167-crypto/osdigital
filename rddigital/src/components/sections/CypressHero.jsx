...
export default function CypressHero() {
    return (
        <Hero
            ...
            titleClass="text-[2rem] md:text-[4rem] leading-[120%] text-center md:text-start lg:leading-[1.3] font-semibold text-[#081F2D]"
            description="We use Cypress to catch issues before your users do. Automated testing gives you confidence with every release. No guesswork. No surprises in production."
            descriptionClass="text-sm text-center md:text-start md:text-lg md:mt-6 md:leading-[1.5] font-normal mt-4 !leading-[175%] tracking-normal capitalize text-[#081F2D]"
            ...
            rightContent={
                <div className="relative h-[30rem] mt-8 lg:mt-0 max-h-72 lg:max-h-full xl:h-full w-full flex items-center">
                    <img loading="lazy" src="https://rd-cms.do.recognisedesign.com/uploads/double_exposure_caucasian_man_virtual_reality_vr_headset_is_presumably_gamer_hacker_cracking_code_into_secure_network_server_with_lines_code_c9822a7c85.webp" alt="Professional businessman working on laptop" className="absolute top-0 left-0 right-0 object-cover h-full w-full z-10 rounded-2xl" title="Cypress Testing Hero Image" />
                    ...
                    <p className="absolute bottom-5 left-6 right-6 z-30 bg-black/20 w-fit rounded-xl py-3 px-4 text-white font-medium text-base lg:text-xl">
                        Fewer bugs, smoother experience for your users!
                    </p>
                    <p className="absolute bottom-[-3.5rem] sm:bottom-[-2.5rem] xl:bottom-[-3.5rem] left-0 lg:left-6 right-0 lg:right-6 z-30 w-fit flex-wrap rounded-xl flex flex-row items-center justify-center gap-x-4 gap-y-0.5 lg:py-3 lg:px-4 text-white font-medium text-base lg:text-xl">
                        {["Faster bug detection", "Zero production surprises", "Consistent reliability"].map((item, i) => (
                            <span key={i} className="flex flex-row items-center gap-2">
                                <svg width="25" height="25" viewBox="0 0 25 25" fill="none" xmlns="http://www.w3.org/2000/svg">
                                    <rect x="0.5" y="0.621216" width="24" height="24" rx="12" fill="#175070" fillOpacity="0.1"></rect>
                                    <path d="M7.83398 14.2878C7.83398 14.2878 8.83398 14.2878 10.1673 16.6212C10.1673 16.6212 13.8732 10.5101 17.1673 9.28784" stroke="#175070" strokeLinecap="round" strokeLinejoin="round"></path>
                                </svg>
                                <span className="whitespace-nowrap text-[#042537] text-xs font-medium lg:text-sm">
                                    {item}
                                </span>
                            </span>
                        ))}
